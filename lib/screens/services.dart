...
void _fetchData() {
  ref.listen<AsyncValue<List<ProductModel>>>(productProvider, (previous, next) {
    if (next is AsyncData<List<ProductModel>>) {
      setState(() {
        products = next.asData?.value; // Correct way to access value
        isLoading = false;
      });
    } else if (next is AsyncError) {
      setState(() {
        errorMessage = next.error.toString(); // Correct way to access error
        isLoading = false;
      });
    }
  });

  ref.listen<AsyncValue<List<ServiceModel>>>(serviceProvider, (previous, next) {
    if (next is AsyncData<List<ServiceModel>>) {
      setState(() {
        services = next.asData?.value; // Correct way to access value
        isLoading = false;
      });
    } else if (next is AsyncError) {
      setState(() {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Product List
            if (!isLoading && products != null && services != null)
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final productServices = services!
                        .where((service) => service.data![0].productIds != null && service.data![0].productIds!.contains(product.data![0]. productId))
                        .toList();
            ...
            if (isExpanded)
              Column(
                children: widget.services.map((service) {
                  return ListTile(
                    title: Text(service.data![0]. name ?? 'Unknown Service'),
                    subtitle: Text('Price: ₹${service.data![0]. price ?? 'N/A'}'),
